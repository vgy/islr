...
+ 3.1 Why is linear regression important to understand? Select all that apply:
	...
	Explanation: The linear model (and every other model) is hardly ever true, but it is an important piece in many more complex methods.
...
+ 3.2 Which of the following are true statements? Select all that apply:
	...
	Explanation: Confidence intervals are a "frequentist" concept: the interval, and not the true parameter, is considered random.
...
+ 1. We run a linear regression and the slope estimate is 0.5 with estimated standard error of 0.2. What is the largest value of b for which we would NOT reject the null hypothesis that β1=b? (assume normal approximation to t distribution, and that we are using the 5% significance level for a two-sided test; need two significant digits of accuracy)
	...
	Explanation: The 95% confidence interval { β1 ± 1.96 S.E.(β1) } contains all parameter values that would not be rejected at a 5% significance level.
		[ref: https://www.reddit.com/r/HomeworkHelp/comments/41d7kl/intro_statistical_learning_cant_solve_hypothesis/]
		B1 - 2 x SE(B1)
		0.5 - 2 x 0.2 = 0.1
		B1 + 2 x SE(B1)
		0.5 + 2 x 0.2 = 0.9
...
+ 2. Which of the following indicates a fairly strong relationship between X and Y?
	...
	- Y : R^2=0.9
	- N : The p-value for the null hypothesis β1=0 is 0.0001
	- N : The t-statistic for the null hypothesis β1=0 is 30
	
	Explanation: 
		The  is the correlation between the two variables and measures how closely they are associated. 
		The p value and t statistic merely measure how strong is the evidence that there is a nonzero association. 
...
True or False? The estimate β^1 in a linear regression that controls for many variables (that is, a regression with many predictors in addition to X1) is usually a more reliable measure of a causal relationship than β^1 from a univariate regression on X1.
	...
	Explanation: 
		Adding lots of extra predictors to the model can just as easily muddy the interpretation of  as it can clarify it. 
		One often reads in media reports of academic studies that "the investigators controlled for confounding variables," but be skeptical! 
...
+ 1. According to the balance vs ethnicity model, what is the predicted balance for an Asian in the data set? (within 0.01 accuracy)
	...
	answer: 512.31
	Explanation: For an Asian, the predicted balance is the intercept plus the Asian ethnicity effect.
...
+ 2. What if TV=250 units?
	Answer: .3039
	Explanation: The effect of an additional unit of radio is .0289 plus .0011 times TV.


#### 3.6 Linear regression in R

**Quiz**
	3.R.R1: What is the difference between lm(y ~ x*z) and lm(y ~ I(x*z)), when x and z are both numeric variables?
		- Y : The first one includes an interaction term between x and z, whereas the second uses the product of x and z as a predictor in the model.
		- N : The second one includes an interaction term between x and z, whereas the first uses the product of x and z as a predictor in the model.
		- N : The first includes only an interaction term for x and z, while the second includes both interaction effects and main effects.
		- N : The second includes only an interaction term for x and z, while the first includes both interaction effects and main effects.
		...
		Explanation:
			...
			The difference is that in the first model, lm processes the "*" operator between variables and automatically includes main effects, 
				whereas in the latter model, the expression inside the I() function is not parsed as a part of the formula, but rather is simply evaluated.
