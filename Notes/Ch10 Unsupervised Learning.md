...
## 10.1 Principle Components

	* Quiz
		+ 10.1.R1
			You are analyzing a dataset where each observation is an age, height, length, and width of a particular turtle. 
			You want to know if the data can be well described by fewer than four dimensions (maybe for plotting), so you decide to do Principal Component Analysis.
			Which of the following is most likely to be the loadings of the first Principal Component?
...
## 10.2 Higher order Principal Component
	...
	* Quiz
		+ 10.2.R1
			...
			We conclude that the first principal component represents overall academic ability, and the second represents a contrast between quantitative ability and verbal ability.
...
## 10.4 Hierarchical Clustering
	...
	+ 10.4.R1
		...
		Explanation
			After cutting the dendrogram at threshold t, we keep all the joins with linkage distance less than t and discard the joins with larger linkage distance.
...
## 10.5 Breast cancer example
	...
	+ 10.5.R1
		...
		Explanation
			...
			The coloring of the cells was based on gene expression, but without the hierarchical clustering step, the heat map would not have looked like anything meaningful.
...
## 10.R Unsupervised in R
	...
	+ 10.R.1
		...
		First, concatenate x and x.test using the rbind functions and perform a principal components analysis on the concatenated data frame (use the "scale=TRUE" option). 
		To within 10% relative error, what proportion of the variance is explained by the first five principal components?

			Answer: 0.3498565
			
			Explanation: use "vars = prcomp(rbind(x,x.test),scale=TRUE)$sdev^2"
	...
	+ 10.R.2
		...
		What is the mean-squared test error if we regress y on the first five principal components, and use the resulting model to predict y.test?
			...
			Explanation: 
				In the actual data generating model for this example, the features may be noisy proxies for a few latent variables that actually drive the response.
...
+ 10.Q.1
	K-Means is a seemingly complicated clustering algorithms. Here is a simpler one:
		...
		(for example, the algorithm tries assigning all of the observations to cluster 1 and 
			it also tries to assign them all to cluster 2 even though those are effectively the same solution).
